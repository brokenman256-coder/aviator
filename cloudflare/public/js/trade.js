(() => {
  ...
  const socket = (() => {
    ...
    function connect() {
      ...
      ws = new WebSocket(`${proto}//${location.host}/ws-trade?token=${encodeURIComponent(token)}`);
  ...
  function drawChart() {
    ...
    const lineColor = rising ? "#2ecc71" : "#ff2d55";
    ...
    grad.addColorStop(0, rising ? "rgba(46,204,113,0.25)" : "rgba(255,45,85,0.25)");
  ...
  // ---------- Socket events ----------
  socket.on("trade:assets", ({ assets: list }) => {
    assets = list;
    if (!activeSymbol && assets.length > 0) activeSymbol = assets[0].symbol;
    renderAssetTabs();
    renderActiveAsset();
  });
